 ...
 *
 */
function hachuShinseiMail($dbConnect, $requestNo, $tokFlg, $filePath, &$subject, &$message) {
	...
	// 件名
	$subject = MAIL_SUBJECT_HEADER . "(" . $requestNo . ")発注申請";
	if ($tokFlg == 1) {
		$subject .= " （特注）";
	}
	...
	$sql .= 	" T_Order_Details";
	$sql .= " WHERE";
	$sql .= 	" OrderID = '" . db_Escape($orderData[0]['OrderID']) . "'";
	$sql .= " AND";
	$sql .= 	" Del = " . DELETE_OFF;
	$sql .= " ORDER BY";
	$sql .= 	" OrderDetID ASC";
	...
	$message = mb_ereg_replace('###APPLINO###', $requestNo, $message);
